...
/**
 * Enum of error codes from the Wink API
 * 
 * @see http://chamberlain.custhelp.com/app/answers/detail/a_id/4922/~/what-are-the-myq-error-codes%3F
 * @author Dan Cunningham
 *
 */
public enum WinkResponseCode {
	 ...
	 * @param desc
	 */
	 private WinkResponseCode(int code, String desc){
		this.code = code;
		this.desc = desc;
	}
	 ...
	 /**
	  * Return a WinkResponseCode from a given code number
	  * @param code
	  * @return
	  */
	 public static WinkResponseCode fromCode(int code){
		 for(WinkResponseCode rc : values()){
			 if (rc.getCode() == code)
					return rc;
			}
			return UNKNOWN;
