			...
			if(node.isArray())
			{
				...
				for(int i = 0;i<arraysize;i++)
				{
					...
					if(devicenode.has("light_bulb_id"))
					{
						...
						String radioType = devicenode.get("radio_type").asText();
						this.lightDevices.put(deviceName,new LightDevice(devicenode));
						logger.info("light_bulb_id: " + Integer.toString(deviceId) + " DeviceName: " + deviceName + " RadioType: " + radioType);
					}
					else if(devicenode.has("binary_switch_id"))
					{
						int deviceId = devicenode.get("binary_switch_id").asInt();
						String deviceName = devicenode.get("name").asText();
						String radioType = devicenode.get("radio_type").asText();
						this.lightDevices.put(deviceName,new LightDevice(devicenode));
						logger.info("binary_switch_id: " + Integer.toString(deviceId) + " DeviceName: " + deviceName + " RadioType: " + radioType);
					}
				}
			}
		}	
...
class LightDevice
{
	...
	//private HashMap<String, Integer> intProperties = new HashMap<String, Integer>();
	private HashMap<String, String> textProperties = new HashMap<String, String>();
	
	//private HashMap<String, Integer> intStates = new HashMap<String, Integer>();
	...
	public LightDevice(JsonNode node)
	{
		...
		if(node.has("device_manufacturer"))
			this.textProperties.put("device_manufacturer",node.get("device_manufacturer").asText());
		if(node.has("model_name"))
			this.textProperties.put("model_name",node.get("model_name").asText());
		if(node.has("manufacturer_device_model"))
			this.textProperties.put("manufacturer_device_model",node.get("manufacturer_device_model").asText());
		//this.DeviceManufacturer = node.get("device_manufacturer").asText();
		//this.ModelName = node.get("model_name").asText();
		//this.ManufacturerDeviceModel = node.get("manufacturer_device_model").asText();
		
		JsonNode valueNode = node.get("last_reading");
		if(valueNode.has("connection"))
			this.boolStates.put("connection", valueNode.get("connection").asBoolean());
		if(valueNode.has("powered"))
			this.boolStates.put("powered", valueNode.get("powered").asBoolean());
		//this.Powered = valueNode.get("powered").asBoolean();
		if(valueNode.has("brightness"))
			this.doubleStates.put("brightness", valueNode.get("brightness").asDouble());
